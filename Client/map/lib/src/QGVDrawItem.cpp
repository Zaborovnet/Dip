...
void QGVDrawItem::refresh()
{
    ...
    if (mQGDrawItemText.isNull()) {
        return;
    }
    if (!isVisible()) {
        mQGDrawItemText->hide();
        return;
    }
    ...
    if (isFlag(QGV::ItemFlag::Highlighted) || isFlag(QGV::ItemFlag::IgnoreScale) ||
        ...
        itemTransformText = QGV::createTransfrom(projAnchorText(), scale, azimuth);
    }
    mQGDrawItem->resetTransform();
    mQGDrawItem->setTransform(userTransform, true);
    mQGDrawItem->setTransform(itemTransform, true);
    //mQGDrawItem->setTransform(itemTransformText, true);
    ...
    //
    mQGDrawItemText->resetTransform();
    mQGDrawItemText->setTransform(userTransform, true);
    //mQGDrawItemText->setTransform(itemTransform, true);
    mQGDrawItemText->setTransform(itemTransformText, true);
    mQGDrawItemText->setVisible(effectivelyVisible());
    mQGDrawItemText->setOpacity(effectiveOpacity());
    mQGDrawItemText->setZValue(effectiveZValue());
    mQGDrawItemText->setAcceptHoverEvents(isFlag(QGV::ItemFlag::Highlightable));
    if (QGV::isDrawDebug()) {
...
QTransform QGVDrawItem::effectiveTransform() const
{
    ...
    return mQGDrawItemText->transform();
...
QPointF QGVDrawItem::projAnchorText() const
{
    return projShapeText().boundingRect().center();
}
...
void QGVDrawItem::onProjection(QGVMap* geoMap)
{
    ...
    //
    if (!mQGDrawItemText.isNull()) {
        if (mQGDrawItemText->scene() != geoMap->geoView()->scene()) {
            onClean();
        }
    }
    if (mQGDrawItemText.isNull()) {
        mQGDrawItemText.reset(new QGVMapQGItemText(this));

        geoMap->geoView()->scene()->addItem(mQGDrawItemText.data());
    }
