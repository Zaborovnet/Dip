...
QPixmap FlagsDemo::createQGVImage()
{
...
void FlagsDemo::onInit()
{
    ...
    const int size = 1000;
    const auto geoPos0= QGV::GeoPos(45.096754, 82.682533);
    const auto geoPos1 = QGV::GeoPos(55.096754, 82.682533);
    const auto geoPos2 = QGV::GeoPos(65.096754, 82.682533);
     ...
     */
    // создание элементов выводимыъх на карту
    const QList<QPair<QString, QGVDrawItem*>> items = {
        ...
        { "Test rectangle",
          new Rectangle(randRect(target, size), QGV::ItemFlag::Movable | QGV::ItemFlag::Transformed, Qt::black) },
        { "Test image",
          new Tringle(geoPos0,0)},
        { "Test image",
          new Tringle(geoPos1,1)},
        { "Test image",
          new Tringle(geoPos2,2)},
     ...
     */
    QImage image0 = createQGVImage().toImage();



    for (auto pair : items) {
        if (pair.first=="Test image")
        {

           auto name = pair.first;

                   auto image0 = pair.second;
                   layer->addItem(image0);
                   selector()->addItem(name, std::bind(&FlagsDemo::setSelected, this, image0, std::placeholders::_1));
...
{
    return QGV::GeoRect(QGV::GeoPos(51.848624, 14.325923), QGV::GeoPos(51.743758, 14.453527)); //точка перенаправления на заданные флаги
}
